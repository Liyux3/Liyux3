## Yuxian Li

ECE @ HKU. I work on robot learning, applied ML, and occasionally build tools to speed up research.

Interested in how to steer AI to continuously learn from messy, heterogeneous physical-world data, and designing the low-level compute infrastructure to scale it.

#### Projects

[Streaming Flow Policy](https://github.com/Liyux3/lerobot_MLP-SFP) ·
[Cross-Survey MH Prediction](https://github.com/Liyux3/cross-survey-mental-health-prediction) ·
[3-Axis MLP-Mixer](https://github.com/Liyux3/analog-cerebellum) ·
[MediBot](https://github.com/Liyux3/MediBot) ·
[Scholar MCP](https://github.com/Liyux3/scholar-mcp)

#### Tools

Python · C++ · PyTorch · scikit-learn · ROS
